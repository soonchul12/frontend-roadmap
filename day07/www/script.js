...
class MoneyTracker {
    ...
    init() {
        console.log('MoneyTracker 초기화 시작');
        
        // 앱에서만 실행되는 강제 새로고침 (무한 루프 방지)
        if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            const hasRefreshed = sessionStorage.getItem('moneyTracker_hasRefreshed');
            if (!hasRefreshed) {
                console.log('네이티브 앱에서 강제 새로고침 실행');
                sessionStorage.setItem('moneyTracker_hasRefreshed', 'true');
                this.forceAppRefresh();
                return; // 새로고침 후 다시 실행됨
            } else {
                console.log('이미 새로고침됨, 정상 초기화 진행');
                sessionStorage.removeItem('moneyTracker_hasRefreshed');
            }
        }
        ...
        // 앱에서 강제 함수 재정의
        this.forceFunctionRedefinition();
        
        // 앱에서 강제 파일 교체
        this.forceFileReplacement();
        ...
        console.log('MoneyTracker 초기화 완료');
    }
    ...
    forceCacheInvalidation() {
        ...
        // Session Storage 완전 삭제
        sessionStorage.clear();
        
        // Local Storage에서 캐시 관련 항목만 삭제
        const keys = Object.keys(localStorage);
        keys.forEach(key => {
            if (key.includes('cache') || key.includes('version') || key.includes('timestamp')) {
    ...
    // 앱 버전 체크 및 강제 업데이트
    checkAppVersion() {
        const currentVersion = '202410213400'; // 현재 버전
        const storedVersion = localStorage.getItem('moneyTracker_appVersion');
        
        console.log('현재 앱 버전:', currentVersion);
        console.log('저장된 버전:', storedVersion);
        
        if (storedVersion !== currentVersion) {
            console.log('새 버전 감지! 강제 캐시 삭제 실행');
            
            // 모든 캐시 완전 삭제
            this.clearAllCaches();
            
            // 새 버전 저장
            localStorage.setItem('moneyTracker_appVersion', currentVersion);
    ...
    // 모든 캐시 완전 삭제
    clearAllCaches() {
        console.log('모든 캐시 완전 삭제 시작');
        
        // 1. Cache API 모든 캐시 삭제
        if ('caches' in window) {
            caches.keys().then(cacheNames => {
                cacheNames.forEach(cacheName => {
                    caches.delete(cacheName);
        ...
        // 2. Session Storage 완전 삭제
        sessionStorage.clear();
        
        // 3. Local Storage에서 캐시 관련 항목만 삭제
        const keys = Object.keys(localStorage);
        keys.forEach(key => {
            if (key.includes('cache') || key.includes('version') || key.includes('timestamp') || key.includes('forceRefresh')) {
                localStorage.removeItem(key);
                console.log('Local Storage 항목 삭제됨:', key);
            }
        });
        
        // 4. Service Worker 완전 제거
        ...
        console.log('모든 캐시 완전 삭제 완료');
    }

    // 앱에서 강제 함수 재정의
    forceFunctionRedefinition() {
        console.log('강제 함수 재정의 시작');
        
        // 카테고리 차트 함수 강제 재정의
        this.drawCategoryChart = function(categoryTotals) {
            console.log('새로운 drawCategoryChart 함수 실행');
            ...
            console.log('새로운 drawCategoryChart 함수 완료');
        };
        
        console.log('강제 함수 재정의 완료');
    }

    // 앱에서 강제 파일 교체
    forceFileReplacement() {
        console.log('강제 파일 교체 시작');
        
        // 모든 CSS 파일을 강제로 다시 로드
        const cssLinks = document.querySelectorAll('link[rel="stylesheet"]');
        cssLinks.forEach(link => {
            if (link.href.includes('.css')) {
                const originalHref = link.href;
                const timestamp = Date.now();
                const newHref = originalHref.split('?')[0] + `?v=${timestamp}&t=${timestamp}&force=${Math.random()}`;
                
                console.log('CSS 파일 교체:', originalHref, '→', newHref);
        ...
        jsScripts.forEach(script => {
            if (script.src.includes('.js') && !script.src.includes('script.js')) {
                const originalSrc = script.src;
                const timestamp = Date.now();
                const newSrc = originalSrc.split('?')[0] + `?v=${timestamp}&t=${timestamp}&force=${Math.random()}`;
                
                console.log('JS 파일 교체:', originalSrc, '→', newSrc);
    ...
    forceAppRefresh() {
        // 앱에서만 실행
        if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            console.log('앱에서 강제 새로고침 실행');
            
            // 모든 스타일과 스크립트 파일에 타임스탬프 추가
            this.addTimestampToAllFiles();
            
            // 즉시 강제 새로고침 (지연 없이)
            console.log('즉시 강제 새로고침 실행');
            window.location.reload(true);
        }
    }

    // 모든 파일에 타임스탬프 추가
    addTimestampToAllFiles() {
        const timestamp = Date.now();
        console.log('타임스탬프 추가:', timestamp);
        ...
        cssLinks.forEach(link => {
            if (link.href.includes('.css')) {
                ...
                console.log('CSS 파일 타임스탬프 추가:', link.href);
            }
        });
        ...
        jsScripts.forEach(script => {
            if (script.src.includes('.js')) {
                ...
                console.log('JS 파일 타임스탬프 추가:', script.src);
            }
        });
    }

    // Capacitor 앱용 네비게이션 함수들
    navigateToAnalysis() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 전역 네비게이션 함수 노출
    window.navigateToAnalysis = () => window.moneyTracker.navigateToAnalysis();
    window.navigateToGoals = () => window.moneyTracker.navigateToGoals();
});
